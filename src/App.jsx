...
export default function App() {
  ...
  useEffect(() => {
    if (!user || isDemoMode) {
      if (isDemoMode && user) {
        // Mock data untuk mode demo jika user "login" (diklik manual)
        setUserData({ name: "Demo User", subscriptionLevel: 1, joinDate: new Date().toISOString() });
        setFiles([
          { id: '1', name: 'Contoh Ebook (Demo)', size: '2MB', reqLevel: 1, url: '#' },
          { id: '2', name: 'Contoh Plugin (Demo)', size: '10MB', reqLevel: 2, url: '#' }
        ]);
      }
      return;
    };
    ...
    }, (err) => console.warn("Profile sync blocked by rules"));
  ...
  const handleAuthSubmit = async (e) => {
    e.preventDefault();
    if (isDemoMode) {
      setUser({ email: email, uid: "demo-uid" });
      setAuthMode('login');
      showToast("Masuk dalam Mode Demo (Tanpa Firebase)");
      return;
    }
    ...
    try {
      ...
      setAuthError("Gagal autentikasi. Periksa koneksi atau config Firebase.");
    }
  ...
  const handleAdminUpdateLevel = async (targetUid, newLevel) => {
    if (isDemoMode) return showToast("Fitur ini perlu Firebase asli");
    try {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 font-['Plus_Jakarta_Sans'] p-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 border border-slate-200">
          {isDemoMode && (
            <div className="mb-6 p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-700 text-[10px] font-bold text-center uppercase tracking-widest">
              ⚠️ Running in Demo Mode (Konfigurasi Firebase Belum Diisi)
            </div>
          )}
          <div className="text-center mb-8">
            <h1 className="text-2xl font-black text-slate-900 mb-1">MemberArea<span className="text-indigo-600">.</span></h1>
            <p className="text-slate-400 text-sm font-medium">{authMode === 'login' ? 'Masuk ke dashboard' : 'Daftar akun member'}</p>
          </div>
          <form onSubmit={handleAuthSubmit} className="space-y-4">
            ...
            <input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="Email" className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all font-bold" required />
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-['Plus_Jakarta_Sans'] flex flex-col md:flex-row">
      <aside className={`fixed md:sticky top-0 left-0 h-screen w-64 bg-white border-r border-slate-200 flex flex-col z-40 transition-transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-6 border-t border-slate-100"><button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-black text-rose-500 hover:bg-rose-50 rounded-2xl transition-all">Keluar</button></div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 md:px-10 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <div className="h-10 w-10 bg-indigo-600 text-white rounded-2xl flex items-center justify-center font-black text-sm">{userData?.name?.charAt(0).toUpperCase() || 'M'}</div>
          </div>
        </header>
        ...
        <main className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full">
            ...
            <div className="space-y-8 animate-fadeIn">
              <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl border border-white/5">
                ...
                <div className="relative z-10 text-center md:text-left">
                  ...
                  <p className="text-slate-400 text-lg">Kelola akun dan akses file produk digital Anda di sini.</p>
                </div>
                <button onClick={() => setActiveTab('files')} className="relative z-10 bg-white text-indigo-900 px-10 py-5 rounded-[1.5rem] font-black shadow-2xl transition-all hover:scale-105 active:scale-95">FILE MASTER</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <StatCard label="Pendaftaran" val={userData?.joinDate ? new Date(userData.joinDate).toLocaleDateString('id-ID') : '-'} icon="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" color="emerald" />
                ...
                <StatCard label="Akses Paket" val={levelNames[currentLevel]} icon="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 21.248a11.955 11.955 0 01-9.618-7.016 11.955 11.955 0 010-9.464A11.955 11.955 0 0112 2.752a11.955 11.955 0 019.618 7.016z" color="amber" />
              </div>
            </div>
          )}
          ...
          {activeTab === 'files' && (
            <div className="animate-fadeIn">
              <h2 className="text-3xl font-black text-slate-900 font-['Outfit'] mb-8">Produk Digital</h2>
          ...
          {activeTab === 'admin' && isAdmin && (
            <div className="animate-fadeIn space-y-10">
              ...
              {adminSubTab === 'users' ? (
                ...
                <div className="p-20 text-center bg-white rounded-3xl border border-dashed border-slate-300 text-slate-400 font-bold">Fitur Kelola Produk Aktif Setelah Firebase Terhubung.</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'profile' && (
            <div className="animate-fadeIn max-w-2xl">
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-xl space-y-8">
                ...
                <button onClick={() => alert("Gunakan Firebase asli untuk mengaktifkan update profil")} className="w-full bg-indigo-600 text-white font-black py-5 rounded-[1.5rem] shadow-2xl shadow-indigo-100 transition-all hover:-translate-y-1">UPDATE PROFIL</button>
              </div>
            </div>
          )}
        </main>
